...
class TestWebDBIntegration:
    ...
    def test_create_card_via_web_read_via_db(self, tclient):
        ...
        assert boards[0].lists[0].cards[0].title == "Web Card"
        assert boards[0].lists[0].cards[0].description == "From web"
    ...
    def test_full_workflow_via_web(self, tclient):
        ...
        assert resp.status_code == 200
        boards = kanban_db.get_boards()
        done_list = boards[0].lists[1]

        # Add subtask
        resp = tclient.post(f"/cards/{card.id}/subtasks", data={"name": "Check step"})
        assert resp.status_code == 200
        boards = kanban_db.get_boards()
        sub = boards[0].lists[0].cards[0].subtasks[0]
        ...
        # Final verification
        boards = kanban_db.get_boards()
        assert len(boards[0].lists[0].cards) == 0  # Todo empty
        assert len(boards[0].lists[1].cards) == 1  # Done has card
        moved_card = boards[0].lists[1].cards[0]
...
class TestWebMCPConsistency:
    ...
    def test_db_operations_do_not_conflict(self, tmp_db_path):
        ...
        assert boards[0].lists[0].cards[0].title == "Card on B1"
        assert boards[1].lists[0].cards[0].title == "Card on B2"
